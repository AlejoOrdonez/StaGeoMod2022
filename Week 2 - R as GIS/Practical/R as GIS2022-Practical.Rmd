...
<p class="text-center"; style = "font-size: 20px">
The text in the green filled box marked as **Question** phrases a question.
</p>
</div>

<div class="alert alert-success"; style = "background-color:White; color:black">
<p class="text-center"; style = "font-size: 20px">
Here you will place the answer to the **question**.
</p>
</div>
...
DK.Biodiv <-  read.csv('Data/occurrence.csv')
```
...
```{r TableIn1}
# Step 1. Which groups where sampled in the dataset?
...
```{r TableIn2}
# Step 1. How many observations where recorded for each group (phylum)?
...
```{r TableIn3}
#Step 1. How many species where recorded for each group (phylum)?
tapply(X = DK.Biodiv$scientificName,
       INDEX = DK.Biodiv$phylum,
       FUN = function(x){length(unique(x))})

#Step 2. How many observations where recorded for each group (phylum)?
tapply(X = DK.Biodiv$scientificName,
       INDEX = DK.Biodiv$phylum,
...
**Observations** is a count of the number of the number of individuals recorded regardless of the species identity. **Number of species** indicate the number of species recorded for a group, without considering the number of recorded individuals of each species.
...
DK.Biodiv$siteID <- paste0(DK.Biodiv$decimalLatitude,"-",DK.Biodiv$decimalLongitude)

# Step 2 Count the number of species per site
Rich.Magnoliophyta <- tapply(X = DK.Biodiv[DK.Biodiv$phylum=="Magnoliophyta","scientificName"],
                             INDEX = DK.Biodiv[DK.Biodiv$phylum=="Magnoliophyta","siteID"],
                             FUN = function(x){length(unique(x))})
# Step 3 Count the number of species per site
Rich.Pinophyta <- tapply(X = DK.Biodiv[DK.Biodiv$phylum=="Pinophyta","scientificName"],
                         INDEX = DK.Biodiv[DK.Biodiv$phylum=="Pinophyta","siteID"],
                         FUN = function(x){length(unique(x))})

# Step 4 Estimate the average No of species Per site
mean(Rich.Magnoliophyta) # Average for Magnoliophyta

mean(Rich.Pinophyta) # Average for Pinophyta
```
...
quantile(x = Rich.Magnoliophyta,
         probs = c(0.025,0.975)) # 95%-CI for Magnoliophyta
quantile(x = Rich.Pinophyta,
         probs = c(0.025,0.975)) # 95%-CI for Pinophyta
```
<div class="alert alert-info">
**Your task:**
...
round(c(mean(Rich.Magnoliophyta)-(qnorm(0.975)*(sd(Rich.Magnoliophyta)/sqrt(length(Rich.Magnoliophyta)))),
mean(Rich.Magnoliophyta)+(qnorm(0.975)*(sd(Rich.Magnoliophyta)/sqrt(length(Rich.Magnoliophyta))))),1)

## 95%-CI for Pinophyta
round(c(mean(Rich.Pinophyta)-(qnorm(0.975)*(sd(Rich.Pinophyta)/sqrt(length(Rich.Pinophyta)))),
mean(Rich.Pinophyta)+(qnorm(0.975)*(sd(Rich.Pinophyta)/sqrt(length(Rich.Pinophyta))))),1)
```
...
The **`quantile()``** function estimate the *sample* percentiles - the value at which a given percentage of the observations fall below. The normal approximation, assumes that the data is normally distributed, and uses this assumption to define the 95%-CI. 
</div>
...
# Step 1 Count the number of species per site per group
Rich.PerSite <- tapply(X = DK.Biodiv$scientificName,
                            INDEX = list(DK.Biodiv$siteID,DK.Biodiv$phylum),
                            FUN = function(x){length(unique(x))})

# Step 2. Plot the Box-plot of log-10 richness
boxplot(x = log10(Rich.PerSite), 
        ylab = "Log10 - Species richness",
        col = rainbow(dim(Rich.PerSite)[2]))

# Plot the Box-plot of log-10 richness Ver 2
boxplot(x = (Rich.PerSite),
        log = "y",
        ylab = "Log10 - Species richness",
        col = rainbow(dim(Rich.PerSite)[2]))
```
...
The Y-axes is in a logarithmic scale, which means a difference in one means one order of magnitude difference between groups 
</div>
...
Rich.PerSite <- as.data.frame(Rich.PerSite)

# Step 2 Define the latitude to each site
Rich.PerSite$Lat <- do.call("c",lapply(strsplit(row.names(Rich.PerSite),"-"),
       function(x){as.numeric(x[1])}))

# Step 3 Define the Longitude to each site
SiteLon <- do.call("c",lapply(strsplit(row.names(Rich.PerSite),"-"),
       function(x){as.numeric(x[2])}))
Rich.PerSite$Lon <- SiteLon

# Step 4 estimate the correlation between richness and Latitude
apply(X = Rich.PerSite[,c("Magnoliophyta","Pinophyta")],
      MARGIN = 2,
      FUN = function(x){
        cor(x = x,
            y = Rich.PerSite$Lat,
...
# Step 5 estimate the correlation between richness and longitude
apply(X = Rich.PerSite[,c("Magnoliophyta","Pinophyta")],
      MARGIN = 2,
      FUN = function(x){
        cor(x = x,
...
## One way is to use the function `projection()`
projection(BiowideSptPnt)
## Another way is to Call the object
BiowideSptPnt

# Step 2. Define the projection
## As the Object has no projection you can just add it as a string
projection(BiowideSptPnt) <- CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs")
...
BiowideSptPnt <- SpatialPointsDataFrame(coords = Rich.PerSite[,c("Lon","Lat")],
...
The object created with *`coordinates()`* removes the coordinates from the data (it only has 13 variables), while the object created with `SpatialPointsDataFrame()` has all the data included in the `Rich.PerSite` object.
</div>
...
```{r SpaPoly1}
# Step 1. Get The level-1 map for Denmark - Denmark/ISO-code is DNK
LandsdeleShp <- getData(name = 'GADM',
                        country= 'DNK',
                        level=1,
                        path = "Data/")
...
NO, The reasons is that the object `LandsdeleShp` is geo-referenced, while `BiowideSptPnt.UTM` is projected to UTM-coordinates.
</div>
...
# Step 2. Extract the region name that corresponds to each sampled point using the function over.
landsdele <- over(x = BiowideSptPnt.UTM, # The SpatialPointDataFrame
                  y = LandsdeleShp.UTM["NAME_1"] # The attribute to extract 
)

# Step 3. add this variable to BiowideSptPnt.UTM
BiowideSptPnt.UTM$landsdele <- as.vector(landsdele[,1])

# Step 3. Check the data in BiowideSptPnt.UTM by Calling the first five rows
head(BiowideSptPnt.UTM)
```
<div class="alert alert-info">
**Your task:**
Using the `BiowideSptPnt.UTM` and `LandsdeleShp.UTM` objects, you will.
...
```{r SpaPoly4}
# Step 1. Estimate the number of sampled sites per region.
SmpSitPerReg <- table(BiowideSptPnt.UTM$landsdele)

# Step 2b. add this variable to LandsdeleShp.UTM
LandsdeleShp.UTM$NoSampSites <- SmpSitPerReg
...
plot(x = LandsdeleShp.UTM[,"NoSampSites"],
     col = rev(hcl.colors(40,palette="reds")[LandsdeleShp.UTM$NoSampSites]),)
box()
# Step 2. Add a legend can be used to interpret the colours representing the number of sampled.  
legend("topright",
       fill = rev(hcl.colors(40,palette="reds")[c(1,10,20,30,40)]),
       legend = seq(0,40,length.out=5))

# Step 3. Estimate the richness of vascular plants
S.Spermatophyte = apply(X = BiowideSptPnt.UTM@data[,c("Magnoliophyta","Pinophyta")],
                        MARGIN = 1,
                        FUN = sum,
                        na.rm=T)
# Step 4. Overlay `BiowideSptPnt.UTM`
plot(BiowideSptPnt.UTM,
     pch=19,
     cex= S.Spermatophyte/median(S.Spermatophyte),
     add = T)

# Step 5. Add a legend that can be used to interpret the species richness per site.  
legend("topleft",
       pch = 19,
       legend = seq(10,135,by=20),
       pt.cex = seq(10,135,by=20)/median(S.Spermatophyte))
```
...
BASEMAP.Crop <- raster("Data/BASEMAP_2018_Crop.tif")
...
`BASEMAP.Crop` is in the UTM/ETRS89 projection
</div>
...
BASEMAP.Legend <- read.csv("Data/Basemap-Legend.csv")

# Step 2. Extract the Data in BASEMAP.Crop and save it as a object named BASEMAP.Data
BASEMAP.Data <- values(BASEMAP.Crop)

# Step 3. Transform BASEMAP.Data to a factor using the data in BASEMAP.Legend to define the levels and labels
BASEMAP.Data.Factor <- factor(x = BASEMAP.Data,
                              levels = BASEMAP.Legend$Code,
                              labels = BASEMAP.Legend$Name)
# Step 4. Use the function table to count the number of cells per land cover type defined in BASEMAP.Data.Factor - Call the object
CountLandCov <- table(BASEMAP.Data.Factor)
...
Agriculture, intensive, temporary crops is the most abundant land cover type.

Airport/runway, Resource extraction, Sea, and Unmapped are not represented in the map.

Forested areas (Fortes + Forest,Wet) cover an area of 36.6811km2 (the sum of the number of cells x the area of each cell (100m2) x the conversion factor from m2 to km2)
</div>
...
```{r RasrRecl3}
# Step 1. Create a reclassification table (Reclas.Table) using the data in BASEMAP.Legend
Reclas.Table <- BASEMAP.Legend
Reclas.Table$Name <- 0
Reclas.Table$Name[grep("Agriculture",BASEMAP.Legend$Name)] <- 1
Reclas.Table <- rbind(Reclas.Table,c(NA,NA))
# Step 2. Transform Reclas.Table into a matrix
Reclas.Table <- as.matrix(Reclas.Table)
# Step 3. Use the reclassify function to transform BASEMAP.Crop into a binary map (BASEMAP.Agri) based on Reclas.Table
BASEMAP.Agri <- reclassify(x = BASEMAP.Crop,
                          rcl = Reclas.Table)
...
```{r RasrRecl4}
# Step 1. Aggregate the raster `BASEMAP.Agri`. Save the result as an object named AgriDen
AgriDen <- aggregate(x = BASEMAP.Agri,
                     fact = 10,
                     fun = sum,
                     na.rm = T)
...
NatDens.DK <- raster("Data/NatDens_BaseMap.tif")
...
The `NatDens.DK` object is projected to Denmark's projection system (UTM/ETRS89).
By comparison `BiowideSptPnt` is only referenced, so it **CAN NOT** be used to extract the information in `NatDens.DK`. You need to use `BiowideSptPnt.UTM` for this.
</div>
...
```{r RasrRecl6}
#Step 1. Extract the data in NatDens.DK & save it to the appropriate SpatialPoints object 
BiowideSptPnt.UTM$NatDens <- extract(x = NatDens.DK,
                                     y = BiowideSptPnt.UTM)

#Step 2. Plot a histogram for *NatDens*
hist(BiowideSptPnt.UTM$NatDens)
```
...
```{r RasrRecl7}
# Step 1. Transform `NatDens.DK` into a factor
BASEMAP.Crop.Fact <- ratify(BASEMAP.Crop)

# Step 2. Extract the Factors - note that you will get a list with one slot.
rat <- levels(BASEMAP.Crop.Fact)

# Step 3. Add the Name for each factors using the information in `BASEMAP.Legend`
rat[[1]][,'landcover'] <- BASEMAP.Legend[match(rat[[1]][,1],BASEMAP.Legend[,1]),2]

# Step 4. Make rat the levels in BASEMAP.Crop.Fact
levels(BASEMAP.Crop.Fact) <- rat

# Step 4. Plot BASEMAP.Crop.Fact
levelplot(x = BASEMAP.Crop.Fact,
          col.regions = rainbow(dim(rat[[1]])[1]))
```
...
Tmean.DK <- stack("Data/Tmean_CHELSA-DK.tif")
...
The difference between `raster()` and  `stack()`/`brick()` is that `raster()` ONLY loads one of the layers in the raster. Both `stack()`and `brick()` can load multiple layers of multi band rasters, or store multiple individual rates ans a multi-band raster object. The difference between `stack()`and `brick()` is that in **stacks** all data is in the same locations, while for **brick** it can be in multiple locations.
</div>
...
```{r RasrMapAlg2}
# Step 1. Estimate the annual Temperature seasonality as SD
TSes.SD <- calc(Tmean.DK,
                fun = function(x){sd(x,na.rm = T)})

# Step 2. Estimate the annual Temperature seasonality as CV
TSes.CV <- calc(Tmean.DK,
                fun = function(x){sd(x,na.rm = T)/mean(x,na.rm = T)})

# Step 3. Stack `TSes.SD` and `TSes.CV` 
TSes.DK <- stack(TSes.SD, TSes.CV)
...
The `TSes.DK` object is referenced, so you CANNOT use `BiowideSptPnt.UTM` to extract the information in `TSes.DK` as `BiowideSptPnt.UTM` is projected to Denmark's projection system (UTM/ETRS89).
</div>
...
```{r RasrMapAlg3}
# Step 1. Re-project the raster 
TSes.DK.UTM <- projectRaster(from = TSes.DK,
...
```{r RasrMapAlg4}
# Step 1. Extract both the SD derived Temperature seasonality  
BiowideSptPnt.UTM$TSes <- extract(x = TSes.DK.UTM[["TSes.SD"]],
                                  y = BiowideSptPnt.UTM)
...
boxplot(formula = TSes ~ landsdele,
        data = BiowideSptPnt.UTM,
        col = rainbow(5))
```
