...
## The first step: Loading the data - 1 
...
## The first step: Loading the data - 2
...
```{r ReadData_Q, echo=FALSE}
quiz(caption = "Answer the next questions based on what is in the table:",
  question("How many integer variables are in the database?",
    answer("5"),
    answer("1"),
    answer("3", correct = TRUE),
    answer("none"),
    incorrect = "Incorrect. There are three numeric values of the class `int`"
    ),
  question("Is `SITE` a factor variable?",
    answer("FALSE", correct = TRUE),
    answer("TRUE"),
    incorrect = "Site is loded as a character string variable.",
    correct = "You got this!, Site is loded as a character string variable."
  )
)
```
...
```{r print-limit-check}
grade_result(
  pass_if(~identical(.result, head(green)))
)
```
...
```{r DataNames1-solution}
names(green)
```

```{r DataNames1-check}
grade_result(
  pass_if(~identical(.result, names(green)))
)
```
...
```{r DataNames2, exercise=TRUE}
# Which value in the dataset has the third Drum site quadrant observation for the `TOTMASS` variable?

```
```{r DataNames2-solution}
green$TOTMASS[c(green$SITE == "DS" & green$QUADNUM == 3)]
```

```{r  DataNames2-hint}
mtcars[c(mtcars$am ==1),"mpg"]
mtcars$mpg[c(mtcars$am ==1),]
```


```{r DataNames2-check}
grade_result(
  pass_if(~identical(.result, green$TOTMASS[c(green$SITE == "DS" & green$QUADNUM == 3)]))
)
```
...
```{r DataNames3-check}
grade_result(
  pass_if(~identical(.result, green[c(green$SITE == "LS" & green$QUADNUM == 2), ]))
)
```
...
```{r Variation1, exercise=TRUE}
# Plot a bar-plot summarizing the number of Quadrants per site.
# Create a object named TotSite that contains the count of observations for each site.
TotSite <- 

# Create a figure with the number of quadrants per site.
------(TotSite)
```

```{r Variation1-solution}
...
```{r  Variation1-hint}
table(mtcars) # to tabulate
barplot(mtcars$mpg) # to make a box-plot
```
...
```{r Variation2, exercise=TRUE}
# Plot a histogram for total biomass for **all** Sites.

```

```{r Variation2-solution}
## Plot a histogram for total biomass for **all** Sites.
hist(green$TOTMASS)
```

```{r  Variation2-hint}
hist(mtcars$mpg) # to make a histogram
```
...
```{r  Variation3-hint}
hist(mtcars$mpg , freq = T) # plots a histogram, the freq argument states if frequencies or densities should be ploted
Dens <- density(mtcars$mpg) # to make a density estimation
lines(Dens) # plot a Density estimation over the histogram
```
...
```{r Variation-Q, echo=FALSE}
quiz(caption = "Based on what you see in the histogram and density plot",
  question("Do the histograms look normally distributed/skewed?",
    answer("Yes"),
    answer("No", correct = TRUE),
    incorrect = "No, they don't. They look bimodal - Two Peaks.",
    correct = "Correct!, They look bimodal - Two Peaks."
  ),
  question("Can you detect the presence of outlines?",
    answer("Yes"),
    answer("No", correct = TRUE),
    incorrect = "No, there are no outliers, That is, there are no extrely large or low values.",
    correct = "Perfect!, there are no extrely large or low values."
  ),
  question("Select the best definition for Histograms AND Density plots [need to make two selections].",
    answer("Histograms and density plots are the same."),
    answer("Histograms count the number of observations.", correct = TRUE),
    answer("Density is a smoothed function of the expected kernel density estimates.", correct = TRUE),
    answer("Histograms are a smoothed function of the expected kernel density estimates.."),
    answer("Density count the number of observations."),
    incorrect = "Histograms count the number of observations on predefined classes, density is a smoothed function of the expected kernel density estimates."
  )
)
```
...
-------- (-------- ~ --------, 
          data = --------) # Name of the object with the data.
```

```{r Boxplot-hint}
# REMEMBER: box-plot illustrate the Min-Max values (lower/upper whisker), inter-quantile range (Box lower/upper edges), and the median (thick centre line).
boxplot (mpg ~ am, # Place what you want to plot here as a formula VarToPlot ~ GroupingVar.
        data = mtcars ) # Name of the object with the data.
```
...
```{r Boxplot-Q, echo=FALSE}
  question("Do you see any difference between the `DS` and `LS` sites?",
    answer("Yes", correct = TRUE),
    answer("No"),
    incorrect = "DS sites have, on average lower biomass.",
    correct = "Correct!, DS sites have, on average lower biomass."
  )
```
...
```{r ScattPlt1, exercise=TRUE}
# Build a scatter-plot showing the relation between total biomass (y-axis) per site and burrows density (x-axis).
------ (------ ~ ------, # Place what you want to plot here as a formula y-var ~ x-var.
     data = ------) # Name of the object with the data.

# Add a resistance line to the scatter plot.
# Create a resistance line object.
ResLn <- line(x = green$------, ## The x-axis variable.
              y = green$------) ## The y-axis variable.
# Plot the resistance line
------ (coef(------))
```

```{r ScattPlt1-solution}
# Build a scatter-plot showing the relation between total biomass (y-axis) per site and burrows density (x-axis).
...
```{r ScattPlt1-hint}
# Scatter-plots are build using the plot function
plot(mpg ~ wt, # Place what you want to plot here as a formula y-var ~ x-var.
     data = mtcars) # Name of the object with the data.
# Create a resistance line object.
ResLn <- line(x = mtcars$wt, ## The x-axis variable.
              y = mtcars$mpg) ## The y-axis variable.
abline(coef(ResLn))
```
...
```{r ScattPlt1-Q, echo=FALSE}
  question("Do you see a relation between total biomass and burrows density?",
    answer("Yes", correct = TRUE),
    answer("No"),
    incorrect = "The plot indicates a negative relationship between these. ",
    correct = "Correct!, The plot indicates a negative relationship between these."
  )
```
...
## Create a resistance line object
ResLn <- ------(x = greenDS$------, ## The x-axis variable
                y = greenDS$------) ## The y-axis variable
# Plot the resistance line
------(------ (------),
       col="red")
...
```{r ScattPlt2-Q, echo=FALSE}
  question("How does the relation between total biomass and burrows density change when each site is compared to all sites?",
    answer("LS sites change SLOWER than DS sites"),
    answer("LS sites change FASTER than DS sites", correct = TRUE),
    answer("LS sites change AS fast than DS sites"),
    answer("No"),
    incorrect = "When the relation between total biomass and burrows density is evaluated for individual sites, the trend shifts from negative (Biomass decreases as density increases) to positive (biomass increases as density increases), with a steeper change for Drum site.",
    correct = "Correct!, When the relation between total biomass and burrows density is evaluated for individual sites, the trend shifts from negative (Biomass decreases as density increases) to positive (biomass increases as density increases), with a steeper change for Drum site."
  )
```

## How "strong" is the covariation? - 1
...
## How "strong" is the covariation? - 2
...
```{r Correlation1, exercise=TRUE, exercise.setup = "Correlation_Setup"}
## Pearson Correlation 
-----(x = -----$-----, # Sets the x-axes range of values
      y = -----$-----, # Sets the y-axes range of values
      method = "-----") # Sets the correlation coefficient to use
```
```{r Correlation1-solution}
## Pearson Correlation 
...
## Pearson Correlation 
cor(x = mtcars$mpg, # Sets the x-axes range of values
    y = mtcars$wt, # Sets the y-axes range of values
    method = "spearman") # Sets the correlation coefficient to use
```

```{r Correlation1-check}
grade_result(
  pass_if(~identical(.result, cor(x = greenDS$BURROWS,y = greenDS$TOTMASS,method = "pearson")))
)
```
## How "strong" is the covariation? - 3

Perhaps things went a bit to fast, you might what to test for the correlation assumptions before you do anything. The figute tells us that the relations are monotonic, but what about normality? here two assumptions are checked, which need to be fulfilled before performing the correlation.
...
```{r Correlation2, exercise=TRUE, exercise.setup = "Correlation_Setup"}
...
## How "strong" is the covariation? - 4
...
```{r Correlation4, exercise=TRUE, exercise.setup = "Correlation_Setup"}
...
```{r Correlation4-Q, echo=FALSE}
quiz(caption = "Based on your asssesments of normality",
  question("Are the variables normally distributed?",
    answer("`Only `TOTMASS` is normaly distributed."),
    answer("Only `BURROWS` is normaly distributed."),
    answer("Both `TOTMASS` and BURROWS` is normaly distributed.", correct = TRUE),
    answer("Neither is  normaly distributed."),
    incorrect = "Both are normaly distributed beacuse the p-value is greater than 0.05. **Why?** because the null-hypothesis tested is if the observed distribution is the same as normal; and if they are the same should be p-value is greater than 0.05.",
    correct = "Correct!, p-value is greater than 0.05, so you can assume the normality for both variables."
  ),
  question("Is it ok to use a Person correlation?",
    answer("Yes.", correct = TRUE),
    answer("No"),
    incorrect = "As both `BURROWS` and `TOTMASS` are normal for the Drum site, Person correlations can be used",
    correct = "Correct!, both `BURROWS` and `TOTMASS` are normal."
  )
)
```
...
```{r Correlation5, exercise=TRUE, exercise.setup = "Correlation_Setup"}
# Estimate the Pearson/Spearman correlations and it's significance - USE THE RIGTH TEST
cor.test(x = greenDS$-----, # Sets the x-axes range of values
         y = greenDS$-----, # Sets the y-axes range of values
         method = "-----") # Sets the correlation coefficient to use
```

```{r Correlation5-solution}
# Estimate the Pearson/Spearman correlations and it's significance - USE THE RIGTH TEST
...
```{r Correlation5-hint}
cor.test(x = mtcars$mpg, # Sets the x-axes range of values
         y = mtcars$wt, # Sets the y-axes range of values
         method = "spearman") # Sets the correlation coefficient to use
```

```{r Correlation5-check}
grade_result(
  pass_if(~identical(.result, cor.test(x = greenDS$BURROWS,y = greenDS$TOTMASS)))
)
```

```{r Correlation5-Q, echo=FALSE}
question("Is the relation between `BURROWS` and `TOTMASS` significant for the `DS` site?",
    answer("Yes."),
    answer("No.", correct = TRUE),
    answer("Neither is  normaly distributed."),
    incorrect = "Because the p-value is larger than 0.05.",
    correct = "Correct!, because the p-value is larger than 0.05."
  )
```
...
```{r Residuals, exercise=TRUE, exercise.setup = "Correlation_Setup"}
# Create a resistance line object
ResLn <- line(x = greenDS$------, ## The x-axis variable
              y = greenDS$------) ## The y-axis variable

# Plot the Residuals vs BURROWS
plot(x = ------$------, # The BURROWS values for the DS site
     y = ------$------, # the residuals of the TOTMASS ~ BURROWS relations for DS sites
...
text(x = ------$------, # The BURROWS values for the DS site
     y = ------$------ - 0.1, # the residuals of the TOTMASS ~ BURROWS relations for DS sites
                                # substract 0.1 so the name is below each point
     labels = ------$------) # Set the Quadrant number for the Names
```
...
```{r Residuals-Q, echo=FALSE}
question("Are there outliers?",
    answer("No."),
    answer("Yes.", correct = TRUE),
    answer("Is not posible to determine that based on the figure."),
    incorrect = "Quadrants 5 and 3 are possible outliers as these show a large deviation from the Zero line.",
    correct = "Correct!, Quadrants 5 and 3 are possible outliers as these show a large deviation from the Zero line."
  )
```
