...
boxplot(mtcars$mpg)
```
```{r Outlier1-solution, exercise.reveal_solution = FALSE}
# Build 3x2 plotting space filled row-wise 
...
question("For which variable can you detect outlier observation?",
  ...
  incorrect = "Incorrect. When you check the box-plots for all evaluated variables, you do not see any outlieres"
)
```
...
```{r Outlier3, exercise=TRUE, exercise.lines=15}
# Build 3x2 plotting space filled row-wise 
...
for (i in --------- ){ # Here you need specify a object `in` range of values 
# Create and object names EvalVar by extracting from paruelo the variables of interest
  EvalVar <-  paruelo[,-]
# Sort the EvalVar object in increasing order for this you need to use the function sort() and the sorting need to me increasing 
  EvalVar <- sort(x = --------, # Variable to sort
                  decreasing = ----) # increasing or decreasing?
# Do a scatter plot of the sorted values using the function plot()
  plot(---------, # Variable to plot
        pch = 19,cex=0.5, # Filled circles of half size
        main = -- ) # add a title to the plot
}
```

```{r Outlier3-solution, exercise.reveal_solution = FALSE}
# Build 3x2 plotting space filled row-wise 
...
question("For which variable can you detect outlier observation?",
  ...
  answer("Lat"),
  answer("None", correct = TRUE),
  incorrect = "Incorrect. There are no outliers. All fall within the trend of the variable."
)
```
...
```{r HomVar2, exercise=TRUE, exercise.lines=25}
# Build 3x2 plotting space filled row-wise 
par(mfrow = c(3,2), mar=rep(4,4))

# initialise your loop using i as an iterator and cycle through PredNames (which is in the memory)
for (i in -----------){ # Here, you need to specify an object `in` range of values 
  ...
  Lm.Mod <- lm(LC3 ~ ., # This is a special way to define a regression in R, where the variable defined as a response is regressed against ALL other variables in the data.frame referred to in the next argument.
               data = -------) # specify the data.frame object with ONLY the
# Extract the residuals from the Lm.Mod object created above using the residuals() function. Save the result into an object named Lm.Resid - for this use the function residuals()
  Lm.Resid <- -------(-------)
  # Extract the fitted/predicted values from the Lm.Mod object created above using the predict() function. Save the result into an object named Lm.Fitt for this use the function predict()
  Lm.Fitt <- -------(-------)
  # Plot the Residuals vs Predicted values
    plot(------- ~ -------,
         ...
         main = ---) # This argument define the text to be added figure title
  # Add a horizontal line using the abline() function at horizontal zero for reference
  --------(-------)
}
```
```{r HomVar2-hint-1}
# Estimate the residuals
lm(mpg ~ .,# This is a special way to define a regression in R, where the variable defined as a response is regressed against ALL other variables in the data.frame referred to in the next argument.
   data = mtcars)
```
```{r HomVar2-hint-2}
# Estimate the residuals
...
```{r HomVar2-solution, exercise.reveal_solution = FALSE}
# Build 3x2 plotting space filled row-wise 
...
grade_code()
```
```{r HomVar_Q1, echo=FALSE}
question("For which variables is the variance of the transformed relative abundance of C3 observations [LC3] homoscedastic? - *Select ALL the variables you consider homoscedastic*.",
  answer("All"),
  answer("MAP"),
  answer("MAT"),
  answer("JJAMAP", correct = TRUE),
  answer("DJFMAP", correct = TRUE),
  answer("Long", correct = TRUE),
  answer("Lat"),
  answer("None", correct = TRUE),
  incorrect = "Incorrect. For MAP, MAT, and LAT, the residual variation is not similar across fitted values."
)
```
...
```{r HomVar1, exercise=TRUE, exercise.lines=15}
...
# Plot the Residuals vs Predicted values
  plot(  ----------- ~   -----------,
      ...
      ylim = range(-----------), # a new argument you will use here is ylim to set the lower and upper limits of the y-axes. For this, you will specify a vector of two-element with the lower and upper limits. This will allow you to make the y-axis symmetric.
      ) 
# Add a horizontal line using the abline() function at horizontal zero for reference  
  -----------(-----------)
```
```{r HomVar1-hint-1}
# Estimate the residuals
lm(mpg ~ cyl disp + hp + drat + wt + qsec,# This is a special way to define a regression in R, where the variable defined as a response is regressed against ALL other variables in the data.frame referred to in the next argument.
   data = mtcars)
```
```{r HomVar1-hint-2}
# Estimate the residuals
...
predict(Model.1)
```
```{r HomVar1-solution, exercise.reveal_solution = FALSE}
# Build a regression model using the lm() function where all six variables predict the relative abundance of C3 observations. # save the model as an object named FullMod
FullMod <- lm(LC3 ~ MAT + MAP + JJAMAP + DJFMAP + LONG + LAT,
              data = paruelo)
FullMod
...
# Plot the Residuals vs Predicted values
  plot(Lm.Resid ~ Lm.Fitt,
       ...
       ylab = "Residuals", # This argument define the text to be added as the y-axis label
       main = "All variables", # This argument define the text to be added figure title
      ylim = range(Lm.Resid), # a new argument you will use here is ylim to set the lower and upper limits of the y-axes. For this, you will specify a vector of two-element with the lower and upper limits. This will allow you to make the y-axis symmetric.
      ) 
# Add a horizontal line using the abline() function at zero for reference  
  abline(h=0)
```
...
grade_code()
```
```{r HomVar_Q2, echo=FALSE}
  question("Is the variance of relative abundance of C3 observations homoscedastic?",
         answer("Yes"),
         answer("No", correct = TRUE),
         incorrect = "Incorrect. the residual variation is not similar across all levels of fitted values (the spread increases as fitted values increase)."
)
```
...
hist(mtcars$mpg)
```
```{r NormRes2-solution, exercise.reveal_solution = FALSE}
# Use the hist() function to display the distribution of the residuals
hist(Lm.Resid)
#
```
...
```
```{r NormRes3-hint}
qqnorm(mtcars$mpg) # produces a normal QQ plot
qqlines(mtcars$mpg) # adds the isoline
```
```{r NormRes3-solution, exercise.reveal_solution = FALSE}
# use the qqnorm() and qqline() functions to plot a qqplot
...
grade_code()
```
```{r NormRes_Q2, echo=FALSE}
question("Based on the q-q plot, is the residual variation normally distributed?",
         answer("Yes", correct = TRUE),
         answer("No"),
         incorrect = "Incorrect. Although there is variability in the theoretical-observed trend, the residuals appear normally distributed."
)
```
...
```
```{r NormRes4-solution, exercise.reveal_solution = FALSE}
# Implement the shapiro.test() function to assess normality of the residuals. Remember that a non-significant test (p>0.05) means that the observed and expected distribution (in this case, normal) are the same.
...
grade_code()
```
```{r NormRes_Q3, echo=FALSE}
  question("Based on this Shapiro–Wilk test, is the residual variation normally distributed?",
         answer("Yes", correct = TRUE),
         answer("No"),
         incorrect = "Incorrect. The residual variation is normally distributed because the p-value (0.2301) is larger than 0.05."
)
```
...
```{r Collin1, exercise=TRUE}
# Estimate the pairwise Pearson correlation coefficients between predictors and present these as a table. For this you will use the cor() function and use the vector you have created (PredNames) to define the variables to use.
-----------(paruelo[,-----------])

```
```{r Collin1-hint}
# Define a vector of variables
VarsUse <- c("mpg", "cyl", "disp", "hp") 
# Estimate the correlations on the selected variables
cor(mtcars[,VarsUse])
```
```{r Collin1-solution, exercise.reveal_solution = FALSE}
...
```{r Collin2, exercise=TRUE}
# using the function `plot()` build a figure where all pairwise combinations between predictors (use predictors here). Use the vector you have created (PredNames) to define the variables to use.
-----------(paruelo[,-----------])
```
```{r Collin2-hint}
# Define a vector of variables
VarsUse <- c("mpg", "cyl", "disp", "hp") 
# plot the biplots for the selected variables
plot(mtcars[,VarsUse])
```
```{r Collin2-solution, exercise.reveal_solution = FALSE}
...
```{r Collin_Q1, echo=FALSE}
  question("Is there collinearity between preditors?",
         answer("Yes", correct = TRUE),
         answer("No"),
         incorrect = "Incorrect. Yes, for pair such as MAP-Long, MAT-LAT, and DJFMAP-LONG, the correltions are rather high (>0.7)"
)
```
...
```{r Collin3, exercise=TRUE, exercise.lines=25}
...
for (i in PredNames){
# Create new data.frame named Tol.DF where the first variable (names Pred) is the one to be tested.
  Tol.DF <- data.frame(Pred = -----------, 
                       paruelo[,-----------[!PredNames%in%i]]# Add the other predictor variables here. 
...
```
```{r Collin3-solution,  exercise.reveal_solution = FALSE}
# Create a vector with only NA values named Tol.Summ to store the tolerance values
Tol.Summ <- rep(x = NA,
...
```{r Collin_Q2, echo=FALSE}
question("Which variable shlud be removed due to high collinearity variable?",
  answer("MAP"),
  answer("MAT"),
  answer("JJAMAP"),
  answer("DJFMAP"),
  ...
  answer("Lat"),
  answer("None", correct = TRUE),
         incorrect = "Incorrect. while DJFMAP is the most collinear variable with a tolerance value of 0.18 (VIF = 5.7), based on the normally used tolerance threshold of 0.1, there is no collinearity."
)
```
...
```{r BivarRel, exercise=TRUE, exercise.lines=25}
# Build 3x2 plotting space filled row-wise 
par(mfrow = c(3,2), mar=c(4,4,2,2))

# Now lets loop across the predictor names (PredNames) to estimate the Tolerance.
for (i  in -----------){
...
# Plot the bivariate relation
  plot(x = Temp.DF[,--], # Call the predictor
       y = Temp.DF[,"----------"], # Call the response (LC3)
...
```
```{r BivarRel-solution, exercise.reveal_solution = FALSE}
# Build 3x2 plotting space filled row-wise 
par(mfrow = c(3,2), mar=c(4,4,2,2)) # here you define the 'margins" - blank space between plotting areas)
...
question("Which variables show a relation with LC3?",
  ...
  answer("Lat"),
  answer("None"),
         incorrect = "Incorrect. Of the predictors, only MAT and latitude show a significant correlation with LC3."
)
...
```{r Independ1-solution, exercise.reveal_solution = FALSE}
# Plot the Residuals vs response variable
  plot(Lm.Resid ~ LC3,
       data = paruelo,
       pch=19, # This argument, as defined, sets the plotting point to a filled dot.
...
question("Do you see any patterns in the residuals?",
  answer("Yes", correct = TRUE),
  answer("No"),
  incorrect = "Incorrect. re is a consistent upward trend in the residuals. This means the observations are not independent of each other - something you can expect if you consider that this is spatial data."
)
```
...
```{r Reg2-solution, exercise.reveal_solution = FALSE}
# Build 2x2 plotting space filled row-wise 
...
```
```{r RegQ1, echo = FALSE}
question("What is represented in this figure? - click all the right answers",
  answer("The upper/lower left panel shows if there is homogeneity in the variances."),
  answer("The upper right panel show that the residuals are mostly normally distributed."),
  answer("The lower right panel show that no value has large leverage (has a strong influence on the regression parameters."),
  answer("The lower right panel show that no value has large leverage (has a strong influence on the regression parameters - but observation 31 has some influence as it has large leverage but is not a problem as it has a low residual), or can be considered an outlier."),
  answer("All of the above", correct = TRUE),
  answer("None of the above"),
  incorrect = "Incorrect. All the points are presnted "
)
```
...
```{r StdzPred1-solution, exercise.reveal_solution = FALSE}
# For ease, generate an object named Stdz.paruelo where you will store `LC3` and the used of predictors
...
```{r StdzPred2, exercise=TRUE, exercise.lines=15}
# use the Stdz.paruelo object to generate a regression object named Stdz.Model1
Stdz.Model <- lm(--------- ~ MAT + MAP + JJAMAP + DJFMAP + LONG + LAT,# the regression formula
...
```{r StdzPred2-solution, exercise.reveal_solution = FALSE}
# use the Stdz.paruelo object to generate a regression object named Stdz.Model1
Stdz.Model <- lm(LC3 ~ MAT + MAP + JJAMAP + DJFMAP + LONG + LAT,# the regression formula
              data = Stdz.paruelo) # Call the object with the Scaled data
...
```
```{r StdzPred3-hint-1}
# Example of an apply function
apply(mtcars,
      MARGIN = 1,
      FUN = sum)
```
```{r StdzPred3-hint-2}
# extract the coefficients of a regressions
coef(Model.1)
```
```{r StdzPred3-solution, exercise.reveal_solution = FALSE}
# Estimate the SD of all the variables - use the apply function for this. Save this in an object named SD.Vars
...
grade_code()
```
```{r StdzPredQ1, echo = FALSE}
question("How do the standarized coeficntes estimates using each aproch compare to eachoter?",
  answer("They are the same", correct = TRUE),
  answer("They are diferent")
  )
```
...
```
```{r SummTbl-hint-2}
# merge vectors one over the other
rbind(mtcars$mpg,
      mtcars$cyl)
# merge vectors one next the other
cbind(mtcars$mpg,
      mtcars$cyl)
...
```{r SummTbl-solution, exercise.reveal_solution = FALSE}
# Generate three objects 
# 1. Store the summary of the regression carried out on original data (FullMod). name this sum.par.lm 
...
```{r Rsqrd, exercise=TRUE, exercise.lines=15}
...
summary(Model.1)
```
```{r Rsqrd-solution, exercise.reveal_solution = FALSE}
# To extract the R^2 and adjusted-R^2 you need to use the `adj.r.squared` and `r.squared` subscripts on an object that builds the summary of an object created with the function `lm`
# r.squared for a regression carried out on original data 
...
question("Does the fit of the model change if you use standardised or the original data?",
  answer("No", correct = TRUE),
  answer("Yes"),
  incorrect = "Incorrect. The fit is not affected by theuse of standarized predictors"
  )
...
```{r MAM1, exercise=TRUE, exercise.lines=15}
...
FrwdSel <- step(--------------, # an object representing a model to simplify
...
```{r MAM1-hint}
# Model reduction using the step function
summary(lm1 <- lm(Fertility ~ ., data = swiss))
summary(lm1) # model before 
slm1 <- step(lm1)
summary(slm1) # model after
```
```{r MAM1-solution, exercise.reveal_solution = FALSE}
# Using the step() function, make a forward stepwise selection on the model with standardised regression coefficients. Save this into a object call FrwdSel
...
```{r MAM2, exercise=TRUE, exercise.lines=15}
...
summary(slm1) # model after
```
```{r MAM2-solution, exercise.reveal_solution = FALSE}
# Using the step() function, make a Backwards stepwise selection on the model with standardised regression coefficients. Save this into a object call BackSel
...
```
```{r MAMQ1, echo = F}
question("Aret he same variables selected by the backwards and forward selection procedures?",
  answer("No", correct = TRUE),
  answer("Yes"),
  incorrect = "Incorrect. The backwards stepwise selection selects a shorter list of variables (JJAMAP, DJFMAP, LAT), while the forward selection includes all variables."
  )
```
```{r MAMQ2, echo = F}
question("Based on the adjusted R-squared, which selection procedure resulted in a better fitting model?",
  answer("backwards", correct = TRUE),
  answer("forward"),
