...
DK.Biodiv <- _________("_________", # file name
...
```{r glmMod1}
DK.Biodiv.lm <- ___(_________ ~ _________, #  select only ONE continuous** predictor.
                    data = _________, # data frame with the data
                    family = "_________" # error distribution
                    )
...
DK.Biodiv.lm.Res <- _________(object = _________, # The regression
                              type = "_________" # the type of residuals which should be returned 
                              )
# Plot the residuals by region as a boxplot
_________(_________~_________, # A formula specifying the variable to pot and the grouping factors
          data = _________) # Data Frame with the data
```
...
```{r glmMod2}
DK.Biodiv.lm2 <- glm(_________ ~ _________ + _________, #  the Equations defining the response and additive combination of predictors
                    data = _________,
                    family = "_________")
...
```{r glmRandInter}
DK.Biodiv.RndInt <- ____(_________ ~ _________ _ _________, #  the Equations defining the response and additive combination of predictors
                         data = _________,
                         family = "_________")
...
TempPred <- seq(from = _________, # the starting values
                to = _________, # the end value
                length.out = ____) # number of object in between

# 2) pair sequential values with regions names
TempPredReg <- data.frame(Slope30mAgg = _________, # sequential predictors values
                          landsdele = rep(_________, # Values to repeat
                                          each = 100) # number of times
                          )
# 3/4) generate a prediction for each predictor/region combination
TempPredReg$S.AllGrp <- _______(_________, # Model
                                newdata = _________, # A data frame in which to look for variables with which to predict.
                                type = "_________" # the type of prediction required
                                )

# Plot a box-plot of the predicted values by region
_________(_________ = _________~_________, # A formula stating the variable to plot and grouping variable 
          data = _________ # a data.frame (or list) from which the variables in formula should be taken
         )
```
<div class="alert alert-success">
...
for (i in _________){
  lines(_________ ~ _________, # A formula stating the variable to plot and grouping variable 
        data = _________,
        col = _________,
        lwd = 2)
}
...
```{r glmMod3}
DK.Biodiv.lm3 <- ___(_________ ~ _________ _ _________, #  the Equations defining the response and additive combination of predictors
                    data = _________,
                    family = "_________")
...
```{r glmRandSlope}
DK.Biodiv.RndSlope <- _____(_________ ~ _________ _ _________, #  the Equations defining the response and additive combination of predictors
                            data = _________,
                            family = "_________")
...
TempPred <- seq(from = _________, # the starting values
                to = _________, # the end value
                length.out = ___) # number of object in between

# 2) pair sequential values with regions names
TempPredReg <- data.frame(Slope30mAgg = _________, # sequential predictors values
                          landsdele = rep(_________, # Values to repeat
                                          each = ___) # number of times
                          )
# 3/4) generate a prediction for each predictor/region combination
TempPredReg$S.AllGrp <- predict(_________, # Model
                                newdata = _________, # A data frame in which to look for variables with which to predict.
                                type = "_________" # the type of prediction required
                                )
# plot the observed Richness Vs predictor 
...
anova(_________, # Model 1
      _________,# Model 2
      test = "_________") # How to contrast models

# AIC fo the models
AIC(_________, # Model 1
    _________) # Model 2
```
...
```{r GAM1}
DK.Biodiv.RndGAM <- gam(_________ ~ _________ + _________, #  the Equations defining the response and additive combination of predictors
                           data = _________,
                            family = "_________")
...
```{r GAM2}
DK.Biodiv.RndGAM2 <- gam(_________ ~ _________ , #  the Equations defining the response and additive combination of predictors
                           data = _________,
                            family = "_________")
