...
# Scatter plot of S.AllGrp vs the evaluated predictor using the test.
 ...
 # Build a linear regression object created with the glm() function.
  ...
  Reg.Tmp <- ------------(------------, # the model formula
                          data = ------------, # Data
                          family = "------------") # Family of the Response
...
# use the function predict()  on the glm model created above to get the expected values 
  ------------(x = ------------, # the ordered predictor
               y = ------------, # the predictions ordered by the predictor
...
# use the functions formula and paste to define the model
  GAM.Tmp <- gam(------------, # the model formula
                 data = ------------, # Data
                 family = "------------", # Family of the Response
                 method ="REML") # The smoothing parameter estimation method.
...
Use all the **CONTINOUS** predictors; but **DO NOT**include `latitude` and `longitude`.
...
DevTable <- sapply(------------, # A vector with the names of the predictors 
       function(Var){

# Build a linear regression object created with the glm()  function.
  GLMM.Tmp <- ------------(------------, # the model formula
                          data = ------------, # Data
                          family = "------------") # Family of the Response
...
# Build a GLM model with (NO PREDICTORS) - the null model
  Null.mod <- ------------(------------, # the model formula
                          data = ------------, # Data
                          family = "------------") # Family of the Response


# Extract the deviances explained per model using the deviance function
  GLMM.Dev.Tmp <- ------------(------------)
  GAM.Dev.Tmp <- ------------(------------)
  Null.Dev.Tmp <- ------------(------------)
  
  # Estimate deviance  explained per model
  GLMM.Dev.Exp <- ------------ # deviance  explained for the GLM
  GAM.Dev.Exp <- ------------ # deviance  explained for the GAM
# Generate a table with the outputs 
    data.frame(Predictor = Var,
               ------------, # Object with deviance  explained for the GLM
               ------------)# Object with-deviance  explained for the GAM
       })
...
DK.Biodiv.gam <- ------------(------------, #  the Equations defining the response and additive combination of predictors
                  data = ------------, # specify the Object where the data is in
                  family = "------------", # Argument defining the adequate error `family` given the type of the response variable (quasi)binomial for presence/absence and proportion data, and (quasi)Poisson for counts.
                  method ="REML") # The smoothing parameter estimation method
...
par(mfcol= c(2,2))
------------(------------, # a fitted gam 
             type = "------------", # type of residuals use deviance
             rep = 500 # How many replicate datasets to generate to simulate quantiles
             )
...
------------(------------, # gam Model with all the predictors
             full = ------------) # Estimate overall concurvity for each smooth. 
...
DK.Biodiv.gam.2 <- ------------(------------ ~ ------------, #  the Equations defining the response and additive combination of predictors
...
par(mfcol= c(2,2))
------------(------------, # a fitted gam 
             ------------ = "------------", # type of residuals use deviance
...
DK.Biodiv.gam.3 <- -update(------------, # the model to update
                           ------------ ) #add the argument for an extra penalty to each term so that it can be penalized to zero
...
------------(------------, # The gam model
             ...
             ------------, # produces a heat-map, with overlaid contours and
             ------------) # produce shaded regions as confidence bands
...
Also, make sure that all rasters have the same extend and number of cells. Those rasters that are misaligned can be corrected using the function `resample()`.
...
Pred.Rast <- -----------(-----------, # Nature Density Raster
                         -----------, # Distance to coast Raster
                         -----------, # HII to coast
                         -----------, # Slope to coast
                         ----------- # Precipitation seasonality to coast
                           )
...
## Here you need to do some operations
