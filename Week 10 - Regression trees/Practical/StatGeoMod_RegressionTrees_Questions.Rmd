...
landsat8 <- _______("_______")

# Change the names of landsat8 using the names() function.
_______(landsat8) <- c("_______",'_______', '_______', '_______', '_______', '_______', '_______')
...
plotRGB(_______[[c(_, _, _)]],# True colour composites flow a RGB order.
        ...
        stretch="_______", # How to stretch the values to increase the contrast?
        margins = TRUE
        )

# Use the plotRGB() function with a linear stretch to make the false colour composites - these follow an a NRI, red, green order.
plotRGB(_______[[c(_, _, _)]],# True colour composites flow a NRI, red, green order.
        r = 1, g = 2, b = 3, # Define which band is red, green, and blue.
        scale = 1, # defined the maximum value a band can take (used to scaling when colouring).
...
image(x = _______, # raster to plot.
     col = _______ # Define the colour ramp 
     )
```
...
rclmat <- matrix(c(_______, _______, 1, # Build
                   _______, _______, 2, # Recreation
                   _______, _______, 3, # Infrastructure
                   _______, _______, 4, # Mining
                   _______, _______, 5, # Agriculture
                   _______, _______, 6, # Nature
                   _______, _______, 7, # Aquatic
                   _______, Inf, NA), # Unmapped
                 ncol=3, byrow=TRUE)

# reclassify the values into seven groups 
BASEMAP2018ReCls <- reclassify(x = _______, # Raster* object
                               rcl = _______ # matrix for reclassification
                               )
...
BASEMAP2018ReCls2 <- _______(x = _______)

## Now define the levels for each value in the ratified RasterLayer
# Create a data.frame with the Raster levels and the Names
rat <- data.frame(ID = _______, # The attributes values.
                  landcover = _______# The name-string for each class
                  )
...
legend("topright", # Define the position the legend.
       fill = _______, # Define the colours of each land cover class.
       legend  = _______, # Define each land cover class name.
...
rasterVis::levelplot(x = _______, # Raster to plot?
                     col.regions = c("grey", # Build 
                                     "light green",# Recreation
                                     "black",# Infrastructure
                                     "red" , # Mining
                                     "orange",# Agriculture
...
set.seed(99)
# Sampling
samp2018 <- _______(x = _______, # Raster to be "sampled"
                             size = _______, # Number of samples per class
...
_______(_______, # Define the SpatialPointsDataFrame to plot
...
sampvals <- extract(x = _______, # Define the raster to be samples.
                    y = _______, # Define the spatial vector data with the locations to sample.
...
sampdata <- data.frame(classvalue = _______, # Get the Landcover "class" for each selected location.
                       _______ # The data.frame with the hyperspectral bands in `landsat8`.
                       )
...
RF.model <- randomForest(formula = as.factor(_______)~., # Define a formula linking the response (land cover classes) to the predictors (bands in landsat8) - remember to Define the Land cover class as a factor
                         data = _______, # Define the data frame containing the variables in the model.
...
# Estimate the randomForest for the evaluated Number of trees
  RF.mode.tmp <- randomForest(formula = _______, # Define a formula linking the response (land cover classes) to the predictors (bands in landsat8) - remember to Define the Land cover class as a factor
                         data = sampdata, # Define the data frame containing the variables in the model.
                         ntree = _______, # Define the number of trees to grow.
                         mtry = _______, # Define the Number of variables randomly sampled at each split. Use the Default = 2.
                         importance = TRUE # Should importance of predictors be assessed?
                  )
# Add the estimated OOB-error to `RF.Error.ntree` - the summary data.frame
  RF.Error.ntree$OOB [_______] <-  RF.mode.tmp$err.rate[dim(RF.mode.tmp$err.rate)[1]]
  }
...
# Estimate the randomForest for the evaluated Number of variables
  RF.mode.tmp <- randomForest(formula = _______, # Define a formula linking the response (land cover classes) to the predictors (bands in landsat8) - remember to Define the Land cover class as a factor
                         data = sampdata, # Define the data frame containing the variables in the model.
                         ntree = _______, # Define the number of trees to grow. Use the Default = 500
                         mtry = _______, # Define the Number of variables randomly sampled at each split. 
                         importance = TRUE # Should importance of predictors be assessed?
                  )
# Add the estimated OOB-error to the summary data.frame
  RF.Error.mtry$OOB [_______] <-  RF.mode.tmp$err.rate[dim(RF.mode.tmp$err.rate)[1]]
  }
# Print the summary data.frame
RF.Error.mtry
...
predRF <- predict(object = _______, # Define the randomForest with the model.
                  newdata = _______, # Define data.frame used to make the prediction - here is the training data.
...
table(_______, # prediction
      _______ # Original values
      )
```
...
k.foldDta <- kfold(_______, # the source data to use (the sample)
                   k = _______, # number of Folds (here you use 5)
                   by = _______) # define how to stratify the folds
...
# Define the Training data.frame - Observations NOT selected in the k-fold.
  train <- sampdata[_______, ]
# Define the test data.frame - Observations selected in the k-fold.
  test <- sampdata[_______, ]
# build a new model using the Train dataset
  RF.model.kfold.Mod <- randomForest(formula = _______, # Define a formula linking the response (land cover classes) to the predictors (bands in landsat8) - remember to Define the Land cover class as a factor
                                     data = _______, # Define the data frame containing the variables in the model.
                                     ntree = 500, # Define the number of trees to grow. Use the Default = 500
                                     mtry = 4, # Define the Number of variables randomly sampled at each split. 
                                     importance = TRUE # Should importance of predictors be assessed?
                                     )
# Predict the k-fold model
  pclass <- predict(object = _______, # Define the randomForest with the model.
                    newdata = _______, # Define data.frame used to make the prediction - here is the training data.
                    type = "response" # indicating the type of output
                    )
# create a data.frame using the reference and prediction
  cbind(Obsered = _______,
         Predicted = _______)
})
...
RF.diag <- diag(_______)
# Overall Accuracy of the RF model
RF.OA <- sum(_______) / _______
RF.OA
```
...
RF.pr2018 <- predict(object = _______, # Define the raster used to make the predictions.
...
sum((_______!= _______)[],na.rm=T)/ncell(_______)
```
...
RFclass.error <- sapply(colnames(_______),
